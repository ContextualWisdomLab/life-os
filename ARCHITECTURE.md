...
Persistence naming migrations preserve normalized relationships and foreign-key ownership. Because PostgreSQL rename DDL can require strong locks, a rename migration must be transactional, bounded by a fail-fast lock timeout, and deployed with the matching service binary rather than across old/new dual writers. The current operational contract is documented in `docs/doctoring/identity-database-semantic-names.md`.
