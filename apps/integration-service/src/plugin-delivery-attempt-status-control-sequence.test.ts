...
function pausedEvidence(
  ...
  return {
    ...
    nextAttemptAt: '2026-09-09T02:56:00.000Z',
    terminalAt: null,
...
function resumedPendingEvidence(): PluginDeliveryAttemptStatusEvidence {
  return {
    ...pausedEvidence(1),
...
class FixedStore implements PluginDeliveryAttemptStatusStore {
  constructor(private readonly result: PluginDeliveryAttemptStatusEvidence) {}
...
describe('PluginDeliveryAttemptStatusApplication control sequence authority', () => {
  it('rejects paused durable evidence with no accepted control transition', async () => {
    const app = new PluginDeliveryAttemptStatusApplication(
      new FixedStore(pausedEvidence(0)),
      () => new Date(CHECKED_AT),
    );
  ...
  it('allows resumed pending evidence to retain a positive control sequence', async () => {
    const app = new PluginDeliveryAttemptStatusApplication(
      new FixedStore(resumedPendingEvidence()),
      () => new Date(CHECKED_AT),
    );

    await expect(app.read(context(), DELIVERY_ID)).resolves.toEqual(
      resumedPendingEvidence(),
    );
  });
});

describe('PostgresPluginDeliveryAttemptStatusStore control sequence authority', () => {
  it('rejects dead-lettered durable evidence with no accepted control transition', async () => {
    const deadLetteredRow = {
    ...
    const client: PluginDeliveryAttemptStatusSqlClient = {
      async query<Row>() {
        return { rows: [deadLetteredRow as unknown as Row], rowCount: 1 };
      },
    };
