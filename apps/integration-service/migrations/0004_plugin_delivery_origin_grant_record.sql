...
CREATE FUNCTION plugin_integration.require_active_plugin_installation_for_delivery_origin()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    WHERE installation_id = NEW.installation_id
      ...
      AND installed_by_user_id = NEW.granted_by_user_id
      AND installation_status = 'active'
      AND revoked_at IS NULL
      AND installed_at <= NEW.granted_at
    FOR SHARE;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'plugin_delivery_origin_active_installation_check'
            USING ERRCODE = '23514',
                  CONSTRAINT = 'plugin_delivery_origin_active_installation_check';
    END IF;
...
CREATE TRIGGER plugin_delivery_origin_active_installation_trigger
BEFORE INSERT ON plugin_integration.plugin_delivery_origin_grant_record
FOR EACH ROW
EXECUTE FUNCTION plugin_integration.require_active_plugin_installation_for_delivery_origin();
