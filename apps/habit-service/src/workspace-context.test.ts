...
function expectProblem(operation: () => unknown, status: number, code: string): void {
  try {
    operation();
  } catch (error) {
...
describe('Habit signed workspace context', () => {
  ...
  it.each([
    { workspaceId: 'not-a-uuid', issuedAt: String(NOW_SECONDS), offset: 0 },
    { workspaceId: WORKSPACE_ID, issuedAt: `0${NOW_SECONDS}`, offset: 0 },
    { workspaceId: WORKSPACE_ID, issuedAt: String(NOW_SECONDS - 61), offset: -61 },
    { workspaceId: WORKSPACE_ID, issuedAt: String(NOW_SECONDS + 6), offset: 6 },
