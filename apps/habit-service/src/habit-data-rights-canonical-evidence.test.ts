...
import { DATA_RIGHTS_CONTRIBUTOR_CONTRACT_VERSION } from './habit-data-rights';
import { parseTrustedHabitDataRightsRequest } from './habit-data-rights-http-boundary';
...
async function expectInvalidRequest(
  ...
  try {
    await parseTrustedHabitDataRightsRequest(
      ...
      BINDING,
      NOW_SECONDS,
      destructive ? { consume: vi.fn().mockResolvedValue(true) } : undefined,
    );
  } catch (error) {
    thrown = error;
  }
  ...
  expect((thrown as HttpException).getResponse()).toEqual({
    type: 'about:blank',
    title: 'Habit data-rights request is invalid',
    status: 400,
    code: 'invalid_data_rights_request',
  });
}

describe('Habit data-rights canonical signed evidence', () => {
  it.each([
    ['workspaceId', WORKSPACE_ID],
    ['requestedByUserId', USER_ID],
    ['requestId', REQUEST_ID],
