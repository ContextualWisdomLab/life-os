...
import { GET, POST } from './[projectId]/tasks/route';
...
function request(method: 'GET' | 'POST', body?: unknown): Request {
  ...
  return new Request(
    `https://life-os.example/api/planning/projects/${PROJECT_ID}/tasks`,
    {
      method,
      headers: {
...
describe('Planning Task Next.js route handlers', () => {
  it('awaits the dynamic Project identifier and delegates GET and POST to the authenticated BFF', async () => {
    ...
    globalThis.fetch = async (input, init) => {
      ...
      const task = {
        ...
        title: 'Wire the Task browser boundary',
        status: 'todo',
    ...
    try {
      ...
      assert.equal(
        (
          await POST(
            request('POST', { title: 'Wire the Task browser boundary' }),
            { params: Promise.resolve({ projectId: PROJECT_ID }) },
          )
        ).status,
        201,
      ...
      assert.deepEqual(planningCalls, [
        { method: 'GET', path: `/v1/projects/${PROJECT_ID}/tasks` },
