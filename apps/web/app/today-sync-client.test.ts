...
describe('Today synchronization BFF', () => {
  it(
    'authenticates the browser, derives workspace server-side, and returns bounded GET state with ETag',
    async () => {
  ...
  it(
    'forwards only the complete Today document and explicit concurrency/idempotency headers on PUT',
    async () => {
      const calls: Array<{ url: string; init: RequestInit | undefined }> = [];
  ...
  it(
    'uses If-None-Match for explicit first migration and does not silently infer overwrite authority',
    async () => {
  ...
  it(
    'passes through only bounded known revision conflicts for explicit reconciliation',
    async () => {
  ...
  it(
    'fails closed on invalid dates, browser bodies, conditional headers, or oversized upstream payloads',
    async () => {
