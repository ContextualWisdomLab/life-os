...
/** Canonicalizes all non-signature evidence descriptors without creating a signature self-reference. */
function canonicalNonSignatureArtifactMetadata(index) {
  return JSON.stringify(
    index.artifacts
      .filter((artifact) => artifact.evidence_type !== 'signature')
      .sort(compareArtifactNames),
  );
}

/** Builds the versioned signed statement that binds release governance and retained evidence semantics. */
function signatureMessage(index, artifact) {
  return Buffer.from(
    [
      ...
      canonicalNonSignatureArtifactMetadata(index),
 ...
 * and rebound to the release-evidence schema, exact source commit, release channel/version,
 * generated timestamp, unresolved P0 buyer-gap set, the canonical metadata of every retained
 * non-signature artifact, and the signed subject name/digest. Binding the complete non-signature
 * descriptor set prevents semantically meaningful evidence metadata such as migration compatibility,
 * evidence type, provenance predicate, size, or digest from being relabeled without invalidating
 * operator authority. Signature descriptors themselves are excluded to avoid a cryptographic
 * self-reference through their own signature bytes and digest.
 *
 * After all signatures pass, the complete normalized evidence set is byte-verified again. That
 * second pass prevents a subject or sibling evidence file changed after the initial artifact pass
 * from being accepted as the retained evidence set merely because its earlier digest was correctly
 * signed. Trust is never inferred from a key embedded in the artifact, a GitHub actor, or model
 * output. The function establishes signature validity only; it does not distribute/rotate trust
 * roots, claim release readiness, or substitute for SBOM, provenance, install, recovery,
 * accessibility, or buyer-journey acceptance gates.
 *
