  ...
  pull_requests: [
    {
      number: 7,
      title:
        'evil ](javascript:alert(1)) <script> sk-abcdefghijklmnopqrstuvwxyz1234567890 @ContextualWisdomLab/security',
      eligible: false,
...
describe('sanitizeUntrustedText', () => {
  it('redacts credentials, escapes control text, and neutralizes mentions', () => {
    const value = sanitizeUntrustedText(
      'name *x* <script> ghp_abcdefghijklmnopqrstuvwxyz1234567890 @example-team'
    );
...
describe('renderCommercialReadinessIssue', () => {
  it('renders one stable marker and no raw review bodies, credentials, or active mentions', () => {
    ...
    assert.equal(markdown.includes('sk-'), false);
  ...
  it('is deterministic for the same report and snapshot', () => {
    const options = { marker: '<!-- life-os-commercial-readiness-loop:v1 -->', maxGaps: 10 };
    assert.equal(
