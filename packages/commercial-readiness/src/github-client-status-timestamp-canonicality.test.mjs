...
it('fails closed when exact-head status timestamp is parseable but not canonical GitHub evidence', async () => {
  ...
  const client = {
    async requestJson(path) {
      ...
      if (path === '/repos/o/r/pulls/7') {
        return {
          number: 7,
          title: 'fix: require canonical status timestamp provenance',
          state: 'open',
          draft: false,
          mergeable: true,
        ...
        return [
          {
            ...
            state: 'failure',
            sha: headSha,
            created_at: '2026-09-06T02:59:00Z',
          },
          {
            id: 200,
            context: 'CodeRabbit',
