  ...
  return {
    async requestJson(path) {
      ...
      if (path === '/repos/o/r/pulls/7') {
        return {
          number: 7,
          title: 'fix: reject coerced review identity authority',
          state: 'open',
          draft: false,
          mergeable: true,
...
it('does not coerce array review identity scalars into exact-head approval authority', async () => {
  const snapshot = await collectRepositorySnapshot(
    fixtureClient({
      user: { login: ['reviewer-a'] },
      state: ['APPROVED'],
      submitted_at: '2026-09-06T04:20:00Z',
      commit_id: [HEAD_SHA],
    }),
    'o/r',
    {
      ...
      generatedAt: '2026-09-06T04:21:00Z',
    },
  );
