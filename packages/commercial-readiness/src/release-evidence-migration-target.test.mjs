...
function releaseIndex(compatibility) {
  ...
  return {
    ...
    version: '0.2.0-rc.1',
    source_commit: SOURCE_COMMIT,
    generated_at: '2026-09-07T14:10:00.000Z',
    open_p0_buyer_gaps: [209, 210],
    artifacts: [
      artifact('life-os-web.oci.json', 'container', containerSha),
      artifact('life-os-migrations.tar', 'migration', digest('2'), { compatibility }),
...
describe('migration compatibility target binding', () => {
  it('accepts only source-version ranges strictly older than the release being produced', () => {
    assert.doesNotThrow(() =>
      validateReleaseEvidenceIndex(
        releaseIndex({
          minimum_source_version: '0.1.0',
          maximum_source_version: '0.1.9',
        }),
      ),
    );
    ...
    for (const compatibility of [
      {
        minimum_source_version: '0.1.0',
        maximum_source_version: '0.2.0-rc.1',
      },
      {
        minimum_source_version: '0.2.0',
        maximum_source_version: '0.2.0',
      },
    ]) {
