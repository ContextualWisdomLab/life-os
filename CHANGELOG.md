...
- The scheduled OpenCode commercial-development loop now proves the reviewed `1.18.9` CLI by spawning the installed binary in an isolated environment and reading `--help` from stderr, so runner `NODE_OPTIONS` or `pnpm exec` cannot present TUI help as the version identity.
