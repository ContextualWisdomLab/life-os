...
- Planning data-rights export now requires persisted Today `revision_number::text` evidence to already be a canonical positive PostgreSQL `bigint` within the signed 64-bit range; zero, negative, padded, reformatted, non-decimal, and overflow values fail closed instead of entering buyer-visible export and digest evidence.
