...
- The scheduled OpenCode commercial-development loop now proves the reviewed `1.18.9` CLI by spawning the installed binary in an isolated environment, checking exact version identity, reading `--pure` from combined help output, and keeping the reviewed `opencode run --help` contract inside that verifier boundary.
- The contextual-orchestrator sidecar now normalizes decimal gateway ports before Bash arithmetic, so leading-zero input cannot acquire octal semantics; executable bootstrap tests cover normalized startup and credential-free missing-provider failure.
...
- The OpenCode development loop now prevents project settings from overriding its reviewed `contextual_orchestrator_gateway/orchestrator/free` route, keeps upstream provider credentials at the gateway bootstrap boundary, records catalog/gateway failures accurately, parses the accepted candidate's exact Compose file outside the model account, and requires digest-pinned PostgreSQL queries plus NATS JetStream probes in pull-request CI.
...
- The commercial-development model account no longer performs Docker commands, never receives Docker-socket authority or upstream provider credentials, and can reach model traffic only through the authenticated loopback contextual-orchestrator gateway.
