...
| Lost update | Stale device overwrites Today plan | Strong revision/precondition semantics on PR #127 | Broader offline domains need separate contracts. |
...
## 9. Data-rights trust boundary

Protected main now provides recent-authentication policy and a durable request/receipt ledger. Complete export/erasure must still fail closed when any required contributor is missing, unprepared, failed or unreconciled. A durable request row is not proof that every domain has completed the right.

## 10. Availability and exhaustion
...
## 11. Residual risks requiring continuing work

- #55 complete data-rights contributor/reconciliation/delivery lifecycle;
- #121/#127 durable Today integration until protected-main merge;
