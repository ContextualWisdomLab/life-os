...
Active #205 exact head `76d6871b3a768f02c211d605b694ef2c6bf0d509` adds host-owned HTTPS delivery-origin authority. Its durable-evidence RED/GREEN lineage now bounds timestamps across installation, grant creation/replay, current reads, and revocation: grants cannot predate installation or be promoted from the future; current reads reject future lifecycle evidence; future-shifted revoke winners fail closed while older durable revocations remain replay-safe. Active grant reads also re-resolve exact installation/workspace/user authority, so installation revocation cannot leave reusable active origin authority; grant evidence predating the current owning installation fails closed. These checks do not introduce network delivery.

Stacked #235 exact head `0f761f1548ecca8467ca887af8aed5b7da6a8838` adds the Integration-owned PostgreSQL grant store. It is a non-force two-parent descendant of its prior child and current #205, inheriting the active-installation read fence while preserving the existing six persistence-owned paths. The adapter rejects malformed SQL result envelopes before row-count/row-array dereference and keeps corrupted runtime persistence evidence inside the bounded `PluginDeliveryOriginPersistenceEvidenceError` contract. Both PRs intentionally perform no outbound HTTP and remain active evidence rather than shipped truth.
