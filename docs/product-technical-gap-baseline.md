...
Stacked #244 exact head `a88c33e730ec187c1f22d928662a5059739558cf` is an ahead-only (`behind_by=0`) descendant of #243 with nine Integration-owned source/test/architecture/ADR/changelog paths. It adds one Integration-owned hosted runtime seam over exactly one SQL pool, rejects generic `DATABASE_URL`, constructs the installation/binding/replay adapters before composing the authenticated Vault operator, and owns concurrency-safe idempotent shutdown. Self-review RED `167667048c5bba32e63e079fa8bd6528722159cd` caught malformed-environment native access and cleanup loss for an acquired malformed pool-like value; repair `b9590f548aa3f4f3e4005220c1e3bd780eec4ff7` bounds both. Fresh RED `a8f2018841fc9dd3f6f170a65b167197a69c18b9` additionally proves that throwing `INTEGRATION_DATABASE_URL` or malformed pool cleanup accessors cannot escape native dependency/configuration detail; repair `a88c33e730ec187c1f22d928662a5059739558cf` bounds property access and captures accepted `query`/`end` methods once behind a stable runtime wrapper. The runtime-owning Nest module and bootstrap ensure the operator is registered before `listen`, validate the listener port before resource acquisition, and close the same runtime on startup failure or application shutdown. Edge-contract commits `0ace60f0bb45c608dc3b88bb5bfd0e3b732adcb0`, `0dad08faab16b4cbe1c789e848b82512ebd237bb`, and `f0b76371c9d19836b1b73493dfd1889ddc70a265` extend the committed tests across malformed database/environment/factory/pool/runtime/application envelopes, listener-port bounds, construction/hook/listener failures, shutdown failure/reuse, and nested cleanup failures. Proposed ADR 0005 remains Proposed. Exact #244 has successful CodeRabbit and Devin Review commit statuses but zero PR-triggered hosted runs on its non-default stacked base, so those statuses are not package/repository/security GREEN or independent approval.

#244 intentionally accepts a PostgreSQL pool factory instead of adding a hidden or cross-service driver. `@life-os/integration-service` still has no direct `pg` dependency/default deployment factory, and the repository installs with `pnpm install --frozen-lockfile`; therefore a package-only `pg` addition without the matching importer lock update is not an acceptable shortcut. This connector runtime also cannot execute the workspace package graph, so the committed RED/repair/edge contracts and ahead-only ancestry are not repository GREEN or executed 100% coverage evidence. The next Plugin dependency is an Integration-owned concrete PostgreSQL driver/default deployment composition with atomic `package.json` + `pnpm-lock.yaml` evidence and real PostgreSQL + Vault acceptance on protected lineage. Only after that gate should #130 advance to separately reviewed SSRF/DNS-rebinding-safe outbound HTTPS with connect-time address enforcement or equivalent reviewed egress, redirect/proxy denial, finite byte/deadline limits, signed/idempotent delivery, durable attempt/outcome/retry/dead-letter evidence, revocation fencing, operator recovery, and buyer-visible status. Do not source-copy Calendar secret storage into Plugin or treat another bounded context's persistence as Plugin authority. #205, #235, #241, #242, #243, and #244 remain active evidence, not shipped truth.
