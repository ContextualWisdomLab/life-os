...
Stacked #235 exact head `7c52c34c6ba06b8ba671fe4e9f3f51fa547e52ff` adds the Integration-owned PostgreSQL grant store. It is a non-force descendant of its prior child and current #205, inheriting the active-installation read fence and code-current parent documentation while preserving the existing six persistence-owned paths. The adapter rejects malformed SQL result envelopes before row-count/row-array dereference and keeps corrupted runtime persistence evidence inside the bounded `PluginDeliveryOriginPersistenceEvidenceError` contract. Both PRs intentionally perform no outbound HTTP and remain active evidence rather than shipped truth.
