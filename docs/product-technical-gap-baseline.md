...
- #245 `77bd65490af6725ec8bd09dfc66f24dacd7b79b1`: Draft concrete node-postgres/default-entrypoint successor, stacked directly on #244 with fresh `behind_by=0` and five Integration-owned changed paths. It keeps the service manifest, concrete Pool adapter/test, and executable entrypoint/test in the Integration bounded context rather than borrowing another service's persistence.
...
Fresh Pool-lifecycle review then found two remaining production-boundary defects: node-postgres documents a default connection-acquisition timeout of zero/no timeout, and idle clients can emit `error` events that must be handled. RED `de075b0940c02bbe7f4ccd12b87bdd8131d8a000` requires explicit finite Pool ownership (`max=10`, `connectionTimeoutMillis=5000`, `idleTimeoutMillis=30000`, `maxLifetimeSeconds=300`) plus a credential-free idle-error listener registered before the Pool crosses the hosted-runtime boundary. Causal repair `733c7f1bad0d00faa44845aaa6df5440bed2e058` implements that boundary. Exact #245 head `77bd65490af6725ec8bd09dfc66f24dacd7b79b1` extends edge coverage so noncanonical classifications and throwing error accessors collapse to bounded `Error` / `null` evidence without native detail. A bounded exact-source Node 22/TypeScript harness verifies this control flow; it is not package/repository or real-database GREEN.
