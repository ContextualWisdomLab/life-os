...
Stacked #235 exact head `7a7b4db7ebee1228469c2217c08db04faf551d21` adds the Integration-owned PostgreSQL grant store and remains an ahead-only descendant of current #205. It now contains seven persistence-owned paths. In addition to canonical persisted UUIDs, exact SQL row-count evidence, invalid-Date handling, non-object row rejection, and malformed SQL-result-envelope rejection, RED `144a6c396ca45485360bcc3472f8b1d317382498` / GREEN `05c1a13c491f31de3a2a58ac6a6ecfa0ae6ab097` makes malformed create/revoke command envelopes fail with bounded persistence validation errors before property access or SQL; `7a7b4db7...` covers null, undefined, primitive, and array envelope branches for both operations. Both PRs intentionally perform no outbound HTTP and remain active evidence rather than shipped truth.
