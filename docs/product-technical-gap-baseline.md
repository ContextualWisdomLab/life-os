...
Stacked #242 exact head `a87509a1de0721456288dececf92ce91091194b6` is an ahead-only (`behind_by=0`) descendant of #241 with four Integration-owned files: a Plugin-owned HashiCorp Vault KV v2 adapter, its focused contract tests, and code-current Architecture/CHANGELOG. It maps one canonical credential-binding UUID to one opaque `lifeos-plugin-vault://` reference and deterministic Vault path, uses create-only `cas: 0`, and accepts an ambiguous/concurrent create replay only when the exact durable binding/installation/workspace/installer/name evidence and secret bytes match. RED `6515c11da843fd923d2958693734a8122319689a` → repair `e9c011b59e6ea67f8055a14d82577d0ec3b28cb3` prevents caller-selected UUID authority from being silently lowercased before Vault path selection. RED `a689429886d0a36a3a7d077579709d4a8df7e331` → repair `56212cc57064721b0a1521f556b93d2fcc9a14a8` keeps the finite request deadline active while replay bytes are consumed. RED `0e0b816672d26b6166763208ccd9f1ebe86ae942` → repair `a87509a1de0721456288dececf92ce91091194b6` replaces post-hoc full-body length checking with incremental `ReadableStream` consumption that cancels as soon as the 65,536-byte limit is exceeded and rejects malformed UTF-8. The adapter remains an active implementation slice only: it is not wired into hosted operator bootstrap, has no real Vault integration acceptance in this connector runtime, and its non-default stacked base does not provide protected-main PR workflow GREEN. #205, #235, #241, and #242 remain active evidence, not shipped truth.
