...
Stacked #244 exact head `e77a79737ebf5a8a338a3366795c42792680faaf` is an ahead-only (`behind_by=0`) descendant of #243 with nine Integration-owned source/test/architecture/ADR/changelog paths. It adds one Integration-owned hosted runtime seam over exactly one SQL pool, rejects generic `DATABASE_URL`, constructs the installation/binding/replay adapters before composing the authenticated Vault operator, and owns concurrency-safe idempotent shutdown. Self-review RED `167667048c5bba32e63e079fa8bd6528722159cd` caught malformed-environment native access and cleanup loss for an acquired malformed pool-like value; repair `b9590f548aa3f4f3e4005220c1e3bd780eec4ff7` bounds both. The runtime-owning Nest module and bootstrap ensure the operator is registered before `listen`, validate the listener port before resource acquisition, and close the same runtime on startup failure or application shutdown. Edge-contract commits `0ace60f0bb45c608dc3b88bb5bfd0e3b732adcb0`, `0dad08faab16b4cbe1c789e848b82512ebd237bb`, and `f0b76371c9d19836b1b73493dfd1889ddc70a265` extend the committed tests across malformed database/environment/factory/pool/runtime/application envelopes, listener-port bounds, construction/hook/listener failures, shutdown failure/reuse, and nested cleanup failures. Proposed ADR 0005 is code-current at exact `e77a797...`, records alternatives, lifecycle evidence, risks, and the remaining driver/acceptance gate, and remains Proposed.
