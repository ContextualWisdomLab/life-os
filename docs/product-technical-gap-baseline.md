...
Active #205 exact head `13847f6942dbcf64bffc34226894f1f5334f94da` adds host-owned HTTPS delivery-origin authority. Its durable-evidence RED/GREEN lineage bounds timestamps across installation, grant creation/replay, current reads, and revocation: grants cannot predate installation or be promoted from the future; current reads reject future lifecycle evidence; future-shifted revoke winners fail closed while older durable revocations remain replay-safe. Active grant reads also re-resolve exact installation/workspace/user authority, so installation revocation cannot leave reusable active origin authority; grant evidence predating the current owning installation fails closed. Architecture and CHANGELOG on the exact head now state the same boundary. These checks do not introduce network delivery.

Stacked #235 exact head `6f0133f440ad890c7daf35361ae9c41185a45ebe` adds the Integration-owned PostgreSQL grant store and remains an ahead-only descendant of current #205. It contains seven persistence-owned paths. In addition to canonical persisted UUIDs, exact SQL row-count evidence, invalid-Date handling, non-object row rejection, malformed SQL-result-envelope rejection, and malformed create/revoke command validation, RED `c5b01e5fa698a5aa5ff66db984634962f1aa0a56` / GREEN `10fb68badfd5fd74a077ad0e461c2622a12d45fa` closes a durable-acceptance ambiguity where `{ rows: [undefined], rowCount: 1 }` was treated as ordinary absence. Coverage head `6f0133f...` also asserts null one-row evidence rejects inside the bounded persistence-evidence contract. Only a structurally valid zero-row result denotes absence. Both PRs intentionally perform no outbound HTTP and remain active evidence rather than shipped truth.
