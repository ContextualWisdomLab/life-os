...
Stacked #244 exact head `fe25f3840ae855900f8b611d8a8ab14ec5dbaecd` is an ahead-only (`behind_by=0`) descendant of #243. It adds one Integration-owned hosted runtime seam over exactly one SQL pool, rejects generic `DATABASE_URL`, constructs the installation/binding/replay adapters before composing the authenticated Vault operator, and owns concurrency-safe idempotent shutdown. Self-review RED `167667048c5bba32e63e079fa8bd6528722159cd` caught malformed-environment native access and cleanup loss for an acquired malformed pool-like value; repair `b9590f548aa3f4f3e4005220c1e3bd780eec4ff7` bounds both. The runtime-owning Nest module and bootstrap ensure the operator is registered before `listen`, validate the listener port before resource acquisition, and close the same runtime on startup failure or application shutdown. Proposed ADR 0005 records alternatives, lifecycle evidence, risks, and the remaining driver/acceptance gate; it is not Accepted.

#244 intentionally accepts a PostgreSQL pool factory instead of adding a hidden or cross-service driver. `@life-os/integration-service` still has no direct `pg` dependency/default deployment factory, and the repository installs with `pnpm install --frozen-lockfile`; therefore a package-only `pg` addition without the matching importer lock update is not an acceptable shortcut. This connector runtime also cannot execute the workspace package graph, so the committed RED/repair contracts and ahead-only ancestry are not repository GREEN. The next Plugin dependency is an Integration-owned concrete PostgreSQL driver/default deployment composition with atomic `package.json` + `pnpm-lock.yaml` evidence and real PostgreSQL + Vault acceptance on protected lineage. Only after that gate should #130 advance to separately reviewed SSRF/DNS-rebinding-safe outbound HTTPS with connect-time address enforcement or equivalent reviewed egress, redirect/proxy denial, finite byte/deadline limits, signed/idempotent delivery, durable attempt/outcome/retry/dead-letter evidence, revocation fencing, operator recovery, and buyer-visible status. Do not source-copy Calendar secret storage into Plugin or treat another bounded context's persistence as Plugin authority. #205, #235, #241, #242, #243, and #244 remain active evidence, not shipped truth.
