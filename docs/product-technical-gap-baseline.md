...
Stacked #241 exact head `b1f163de75e99d2c59de897dc79171824449c905` is an ahead-only descendant of #235 with `behind_by=0` and 15 Integration-owned credential/persistence/migration/test/documentation paths. Its earlier lineage bounds malformed command/context envelopes, invalid server clocks, falsey read evidence, malformed durable rows, SQL result ambiguity, and credential INSERT versus installation-revocation TOCTOU. RED `a4b3e29c8f0233ae2c3613bff9b66a6a40db9029` / repair `b1f163de75e99d2c59de897dc79171824449c905` closes the remaining credential chronology gap: `bind` rejects malformed or future installation time, `active` installation evidence with revocation metadata, durable bindings dated before their installation, and existing/create-winner binding evidence dated after the current operation. `revoke` rejects a binding that is not yet current and future-dated durable revocation replay before provider deletion, while an older exact revoked replay remains valid for idempotent cleanup. A bounded Node 22 control-flow reproduction shows the predecessor accepting these future/revocation states and the repaired predicates rejecting them. Because #241 targets the non-default #235 branch, GitHub reports zero PR-triggered hosted runs on this exact head; that absence is not GREEN and repository/security/database acceptance must be reacquired after normal parent integration/restack. #205, #235, and #241 remain active evidence, not shipped truth.
