...
- #245 `2c46749360260281c3e4e3754bcb28d0bcb92b91`: Draft concrete node-postgres/default-entrypoint successor, stacked directly on #244 with `behind_by=0` and seven Integration-owned source/test/architecture/changelog/manifest paths. It keeps the service manifest, concrete Pool adapter/test, executable entrypoint/test, and code-current architecture/changelog in the Integration bounded context rather than borrowing another service's persistence.
...
A fresh review of that listener found the classification whitelist itself still too permissive: the predecessor accepted any 1–64 character `[A-Za-z0-9_.-]` string, so an arbitrary credential-shaped value could become retained log evidence despite the credential-free contract. RED `e235a50939ad665bea38d158ec734862cbc32e55` demonstrates that `vault_token_ABCDEF1234567890` / `token` crosses the predecessor classifier. Causal repair `2c46749360260281c3e4e3754bcb28d0bcb92b91` retains only the exact non-secret error classes `Error` or `DatabaseError` and canonical five-character uppercase/digit PostgreSQL SQLSTATE evidence; every other classification collapses to `Error` / `null`. PostgreSQL 18 Appendix A is the primary traceability source for the five-character SQLSTATE contract. A bounded Node 22 predicate reproduction verifies predecessor versus repair behavior; this is not package/repository or real-database GREEN.
