...
## 8. Durable Today operations

**Status:** Implemented on protected main

PR #127 merged durable Today workspace synchronization on `f4cae6d83eadb00019d2962a650c55c59a3349ae`. Operators should treat planning PostgreSQL as the authoritative state, preserve migration order, and diagnose stale-write/idempotency conflicts through bounded revision/evidence identifiers rather than inspecting personal Today content in logs. Browser-local drafts are not server backup artifacts until the user explicitly saves them.

## 9. Data-rights operations

**Status:** Partial

Protected main includes recent-authenticated request handling foundations and a durable identity-owned request/receipt ledger. Issue #55 still requires complete contributor orchestration, durable reconciliation, operator-visible stuck requests, retention/legal-hold and protected export delivery.
